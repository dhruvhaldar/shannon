...
## 2024-05-29 - np.empty_like vs np.empty Performance
**Learning:** In NumPy, calling `np.empty_like(arr)` carries unexpected overhead, likely related to safely extracting shape and dtype from arbitrary objects and handling subclassing logic. Explicitly allocating the array using `np.empty(arr.shape, dtype=arr.dtype)` completely bypasses this logic and is consistently faster (~30-50% speedup for array allocation). For `shannon/ground_station.py` in the hot loop, changing to `np.empty` sped up calculation.
**Action:** When allocating uninitialized arrays modeled after existing arrays inside performance-critical loops, avoid `np.empty_like(arr)`. Instead, extract the shape/size and dtype directly from the array properties and use `np.empty(arr.shape, dtype=arr.dtype)`.
...
## 2026-06-03 - NumPy In-Place Operations for Trigonometry and Conditionals
**Learning:** When performing trigonometry (`np.arcsin`, `np.arctan2`) on large numpy arrays, relying on the return value allocates a new array, increasing memory bandwidth overhead. Using the `out` parameter to operate in-place avoids this. Additionally, conditional additions using boolean masking (`arr[arr < 0] += 360.0`) are slower because they require evaluating the condition, allocating a boolean array, indexing, and applying. Using `np.add(arr, 360.0, out=arr, where=arr < 0)` performs the entire operation in-place without generating a separate boolean array, providing another significant speedup. Together these save around 40% time in critical vector loops. However, the `out` parameter causes a `TypeError: return arrays must be of ArrayType` if the inputs are 0-dimensional scalars, so branching logic is required.
**Action:** When working with large arrays, use `out` parameters on ufuncs like `np.arctan2` and `np.divide`. Use `where` parameter in combination with `out` on operations like `np.add` to replace boolean array index assignment (e.g. `arr[arr < 0] += val`). Always guard `out` parameter usages with `if np.ndim(arr) != 0` to safely handle scalar values.
## 2026-06-06 - FastAPI Large Array Serialization Bottleneck
**Learning:** Returning large, simple numerical lists (like flattened float64 arrays) from a FastAPI endpoint using standard response dictionaries triggers the default `jsonable_encoder`. This recursive encoder is notoriously slow for large arrays because it performs `isinstance` checks on every single element to ensure JSON serialization, resulting in severe CPU bottlenecks and multi-second latency for megabyte-sized payloads.
**Action:** When serving large, pre-flattened numerical arrays via FastAPI, explicitly return `fastapi.responses.JSONResponse(content={"data": array_list})` to bypass the slow `jsonable_encoder` validation loop and directly leverage standard library `json.dumps()`.

## 2026-06-08 - Numpy Loop and FastAPI jsonable_encoder Optimization
**Learning:**
1. When constructing a list of dictionaries from multiple parallel numpy arrays inside a loop (like pass prediction points), extracting scalar values from numpy arrays individually inside the loop introduces severe indexing overhead. Using a list comprehension with `zip` and explicitly converting the arrays to python lists first via `.tolist()` yields over a 2x speedup.
2. When a FastAPI endpoint returns a large list of dictionaries containing `datetime` objects by relying on the default framework serialization loop (which uses `jsonable_encoder`), it recursively evaluates `isinstance` on every field of every dictionary to ensure JSON compatibility. This is extremely slow for long lists. Pre-formatting the datetimes using `.isoformat()` and explicitly returning a `fastapi.responses.JSONResponse` circumvents the slow validation loop, resulting in a >5x speedup.
**Action:** Always pre-convert numpy arrays to lists (`.tolist()`) when iterating over them element-by-element to create Python objects. For FastAPI endpoints returning large nested structures or lists with non-native JSON types (like `datetime`), manually format the data into primitives and return a `JSONResponse` directly to bypass `jsonable_encoder` overhead.
